...
function ProjectCard(props) {
	...
	return (
		...
		<div className={profileClassName + " ProjectCard"}>
			...
			<h3 className={profileClassName + " ProjectCardTitle"}>{card.title}</h3>
			<h4 className={profileClassName + " ProjectCardSummary"}>{card.summary}</h4>
			<div className={profileClassName + " ProjectCardBody"}>
				{card.body.map(p => <p>{p}</p>)}
			</div>
			{card.tags.map(t => <div className={profileClassName + " ProjectCardTag"}>{t}</div>)}
